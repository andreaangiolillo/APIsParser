...
module.exports = function (){
  ...
  filter.filter = function ( untouchedNodes, untouchedProperties ){
    ...
    var serviceElements = filterServiceElements(nodes.concat(properties));

    if ( enabled ) {
      setColorsForExternals(externalElements);
      setColorsForServices(serviceElements);
    } else {
      resetBackgroundColors(externalElements);
      resetBackgroundColors(serviceElements);
    }
  ...
  function filterServiceElements( elements ){
    return elements.filter(function ( element ){
      if ( element.visualAttributes().indexOf("deprecated") >= 0 ) {
        // deprecated is the only attribute which has preference over external
        return false;
      ...
      return element.attributes().indexOf("service") >= 0;
    });
  }
  ...
  function setColorsForServices( elements ){
    COLOR_MODES_SERVICE = [
      { type: "same", range: [d3.rgb("#ff0000"), d3.rgb("#ff0000")] },
      { type: "gradient", range: [d3.rgb("#ff0000"), d3.rgb("#ee2849")] } // taken from LD-VOWL
    ];
    ...
    var colorScale = d3.scale.linear()
      .domain([0, entries.length - 1])
      .range(_.find(COLOR_MODES_SERVICE, { type: colorModeType }).range)
      .interpolate(d3.interpolateHsl);
